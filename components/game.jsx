...
function bet() {
    ...
    const [alreadyScanned, setAlreadyScanned] = useState(false); // add state variable
    const [showAlreadyScannedModal, setShowAlreadyScannedModal] = useState(false); // add state variable
    ...
    const handleBarcodeSubmit = async (event) => {
        ...
        try {
            ...
            setShowModal(true);
            if (response.data.alreadyScanned) { // check if already scanned
                setAlreadyScanned(true);
            } else {
                setShowAlreadyScannedModal(true);
            }
        } catch (error) {
            console.error(error);
        }
    ...
    const handleBarcodeAlreadyScannedClose = () => {
        setShowAlreadyScannedModal(false);
                ...
                <form onSubmit={handleBarcodeSubmit}>
                    ...
                    <div className=''>
                        <input type="text" value={barcode}
                            ...
                            placeholder='Scan or Enter the barcode' className='bg-white  border-4 border-green-500 text-center h-[50px] mt-5 ml-4 w-[240px] rounded-2xl ' />
                    </div>
                </form>
                <Modal isOpen={showModal} onClose={() => setShowModal(false)} winningAmount={winningAmount} />

                {alreadyScanned && (
                    <Modal isOpen={showModal} onClose={() => setShowModal(false)} winningAmount={"Already Scanned!"} />
                )}
